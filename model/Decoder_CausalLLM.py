...
class DecoderLlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(self, Z_hidden, home_loc_ids, traj_target):     # [batch_size,z_hidden_size], [batch_size, 1], [batch_size, 24]
        ...
        for batch_idx in range(traj_target.shape[0]):
            home_loc_id = home_loc_ids[batch_idx]
            traj_seq = traj_target[batch_idx]
            inputs = self.prompt.get_prompt(home_loc_id, traj_seq, self.config.traj_length)
        ...
        for i in range(labels.size(0)):
            pre_idx_list = torch.where(labels[i] == self.config.pre_token_id)[0]
            if len(pre_idx_list) > 0:
                labels[i, :pre_idx_list[0] + 2] = IGNORE_INDEX

            start_idx_list = torch.where(labels[i] == self.config.start_token_id)[0]
            ...
            cur_new_input_embeds = torch.cat((inputs_embeds[i][:start_idx + 1],
                                              ...
                                              inputs_embeds[start_idx + 2:]), dim=0)
            new_input_embeds.append(cur_new_input_embeds)
        inputs_embeds = torch.stack(new_input_embeds, dim=0)

        attention_mask = (labels != self.tokenizer.pad_token_id).long()
        return super(DecoderLlamaForCausalLM, self).forward(input_ids=None,
    ...
    def initialize_tokenizer_and_embedding_resize(self):
        ...
        self.config.hidden_token_id, self.config.pre_token_id, self.config.patch_token_id, \
